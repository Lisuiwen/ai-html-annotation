...
- This file provides repository-wide generic visual Tokens and component constraints only; it does not record any specific Case's system names, business fields, page structure, coordinates, measured dimensions, or state copy.
- Screenshots, requirements, page states, business data, and confirmed special visual facts for the current Case must be supplied explicitly in Case materials; do not treat this file as a design spec for a specific business page.
...
Component- or Pattern-specific dimensions do not belong in the foundation. For example, Header, Sider, Search, Filter, Toolbar, Table, Button, and Page Header dimensions and shadows are declared in their respective `component.html` or `pattern.html`. When the current Case provides confirmed overrides, keep the same named references consistent in the final prototype; do not substitute "close enough" color values.
...
- Top navigation height, brand area width, nav item states, and overflow behavior must come from the current Case materials; when not provided, keep a minimal static structure.

### 3.2 Side navigation, tree, and tabs

- Sidebar width, menu hierarchy, tree node size, search placement, and tab states must come from the current Case materials.
...
- Regular filter control widths and filter item counts must be confirmed in current Case materials.
...
- Filter fields, row count, expand/collapse, action order, and transitions must come from current Case materials.
...
- Empty state copy, illustration, trigger conditions, column widths, and layout must come from current Case materials.
...
- Whether to use elevated shadow should be confirmed in current Case materials.
...
- Do not replace hierarchy confirmed in current Case materials with floating card shadows.
- Do not write responsive breakpoints, mobile layout, animation duration, or collapsed widths as formal system spec without current Case confirmation; if the prototype must show them, mark with `ponytail:` as prototype tolerance and replace after real evidence.
