...
test('Direct Edit activate/release manages owner via shared Picker', async () => {
...
test('dirty session blocks element switch and notifies', async () => {
  const env = await boot({ dirty: true }); env.window.AuthorToolsEditTool.activate(); const onSelect = env.calls[0][1].onSelect;
  assert.equal(onSelect({ tagName: 'DIV', id: 'a', className: '' }), true); assert.equal(onSelect({ tagName: 'DIV', id: 'b', className: '' }), false); assert.match(env.toasts.at(-1), /save or cancel current edits first/);
});

test('save without stable selector does not send write request', async () => {
...
test('stable selector save posts patch to edit route', async () => {
...
test('Direct Edit handles IME without full render per key', async () => {
