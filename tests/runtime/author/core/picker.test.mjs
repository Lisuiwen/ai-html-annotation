...
async function boot(platform = 'Win32') {
  const source = await readFile(sourceUrl, 'utf8'); const listeners = new Map(); const windowListeners = new Map(); const body = element('body'); const html = element('html'); body.parentElement = html;
...
test('stableSelector 优先 id、note target，再降级 cssPath', async () => {
  const { window, body } = await boot(); const byId = element('div', body); byId.id = 'foo:bar'; assert.equal(window.AuthorToolsPicker.stableSelector(byId), '#foo\\:bar');
